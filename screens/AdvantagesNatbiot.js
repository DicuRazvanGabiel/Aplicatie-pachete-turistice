...
const AdvantagesNatbiot = ({ navigation }) => {
    ...
    const fetchAdvantages = async () => {
        ...
        const response = await fetch(
            `https://natbiot-travelling-d0a35.firebaseio.com/flamelink/environments/production/content/avantajeleServiciilorNatbiot/en-US.json`
        );
...
AdvantagesNatbiot.navigationOptions = ({ navigation }) => {
    let lang = store.getState().language.language;
    let translate = Translate.avantaje[lang];
    return {
        title: translate,
    };
};
